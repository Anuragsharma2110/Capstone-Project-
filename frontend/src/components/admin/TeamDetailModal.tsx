...
interface TeamDetailModalProps {
    ...
    onRegenerateCredentials: (teamId: number) => Promise<void>;
}

type ActivePanel = 'roster' | 'assign' | 'move' | 'credentials';

const TeamDetailModal: React.FC<TeamDetailModalProps> = ({
    isOpen, team, allTeams, unassignedLearners, defaultPanel, onClose,
    onRename, onRemoveLearner, onMoveLearner, onAssignLearner, onRegenerateCredentials,
}) => {
        ...
        <div className="modal-overlay">
            <div className="modal-container" style={{ maxWidth: '580px' }}>
                ...
                <div style={{ display: 'flex', gap: '0.25rem', padding: '0.75rem 1.5rem 0', background: 'var(--bg-card)' }}>
                    ...
                    <button style={tabStyle(panel === 'credentials')} onClick={() => setPanel('credentials')}>
                        🔑 Credentials
                    </button>
                </div>
                ...
                <div className="modal-form" style={{ paddingTop: '1rem' }}>
                    ...
                    {panel === 'credentials' && (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                            <div style={{ padding: '1rem', background: 'rgba(37,99,235,0.05)', border: '1px solid rgba(37,99,235,0.15)', borderRadius: '10px' }}>
                                <p style={{ margin: '0 0 0.5rem', fontSize: '0.8rem', color: 'var(--text-secondary)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Shared Login Account</p>
                                <p style={{ margin: '0 0 0.75rem', fontSize: '0.82rem', color: 'var(--text-muted)' }}>
                                    All students in this team use one shared account to log in.
                                </p>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', background: 'var(--bg-page)', padding: '0.6rem 0.9rem', borderRadius: '8px', border: '1px solid var(--border-color)' }}>
                                    <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="var(--primary-blue)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                                        <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" /><circle cx="12" cy="7" r="4" />
                                    </svg>
                                    <div>
                                        <p style={{ margin: 0, fontSize: '0.72rem', color: 'var(--text-muted)', fontWeight: 600 }}>USERNAME</p>
                                        <code style={{ fontSize: '0.9rem', fontFamily: 'monospace', color: 'var(--primary-blue)', fontWeight: 700 }}>
                                            {team.team_username ?? `cohort${team.cohort_details.id}_team${team.id}`}
                                        </code>
                                    </div>
                                </div>
                                <div style={{ marginTop: '0.75rem', padding: '0.6rem 0.9rem', background: 'rgba(245,158,11,0.06)', border: '1px solid rgba(245,158,11,0.2)', borderRadius: '8px', fontSize: '0.78rem', color: '#d97706' }}>
                                    ⚠ The password is not shown here for security. Use <strong>Regenerate Password</strong> below to get a new one.
                                </div>
                            </div>
                            <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                                <button
                                    onClick={async () => {
                                        ...
                                        await onRegenerateCredentials(team.id);
                                        setRegenerating(false);
                                    }}
                                    disabled={regenerating}
                                    style={{ background: '#2563eb', color: 'white', border: 'none', padding: '0.5rem 1.25rem', borderRadius: '8px', cursor: 'pointer', fontWeight: 600, fontSize: '0.85rem', display: 'flex', alignItems: 'center', gap: '0.5rem', opacity: regenerating ? 0.6 : 1 }}
                                >
